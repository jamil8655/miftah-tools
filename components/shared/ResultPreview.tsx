...
import { Download, RefreshCw, Copy, Check, CheckCircle2, Share2, Zap, Eye, FileCheck2, ArrowRight, Layers, Sparkles } from 'lucide-react';
...
import { openDownloadedFile, shareDownloadedFile } from '@/lib/utils/download';
...
export function ResultPreview({
  ...
  const seo = tool ? getCompleteToolSeo(tool) : null;
  ...
  const handleShare = async (file?: ResultPreviewProps['files'][0]) => {
    ...
    if (typeof window !== 'undefined') {
      const shareData = {
        title: 'Miftah Tools',
        text: file ? `Processed file: ${file.name} with Miftah Tools` : 'Processed my files with Miftah Tools - Fast, private and free!',
  ...
  return (
    ...
    >
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  ...
                  {/* Direct Share Processed File */}
                  <button
                    type="button"
                    onClick={() => handleShare(file)}
                    className="px-3 py-2 sm:py-2.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/60 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/80 text-xs font-bold inline-flex items-center gap-1.5 transition-all active:scale-95 shadow-xs"
                    title={loc.shareFile}
                  >
                    <Share2 className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                    <span className="hidden sm:inline">{loc.shareFile}</span>
                  </button>
                </div>
              </div>
      ...
      {/* Related Digital Tools & Utilities directly on Result Screen */}
      {relatedTools.length > 0 && (
        <div className="pt-5 border-t border-slate-200/90 dark:border-slate-800 space-y-3.5 text-left rtl:text-right animate-in fade-in duration-300">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-brand-500/10 dark:bg-brand-500/20 text-brand-600 dark:text-brand-400 flex items-center justify-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
            {relatedTools.slice(0, 3).map((rel) => (
              <Link
                key={rel.id}
                href={`/tools/${rel.slug}/`}
                className="p-3.5 rounded-2xl bg-slate-50/90 dark:bg-slate-950/80 hover:bg-brand-50/60 dark:hover:bg-slate-800 border border-slate-200/80 dark:border-slate-800/80 hover:border-brand-500/50 transition-all flex items-center justify-between gap-2.5 group shadow-2xs"
              >
                <div className="min-w-0 space-y-0.5">
                  <span className="text-xs font-black text-slate-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors block truncate">
                    {rel.name}
                  </span>
                ...
                <div className="w-7 h-7 rounded-xl bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/70 flex items-center justify-center shrink-0 group-hover:border-brand-500/50 group-hover:translate-x-0.5 rtl:group-hover:-translate-x-0.5 transition-all">
                  <ArrowRight className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400 group-hover:text-brand-600 dark:group-hover:text-brand-400 rtl:rotate-180" />
                </div>
              </Link>
            ))}
          </div>
